rules = '''plaid fuchsia bags contain 5 light violet bags, 1 light yellow bag.
...
for line in rules.splitlines():
    container, items = line[:-1].split(' bags contain ')
    if items == 'no other bags':
        items = []
    else:
        items = [i[:-4].strip().split(' ', 1) for i in items.split(', ')]
    bags[container] = [(i[1], int(i[0])) for i in items]

def can_contain(container, contained):
    items = [i[0] for i in bags[container]]
    return (contained in items) or any(can_contain(i, contained) for i in items)

print(len(list(filter(lambda b: can_contain(b, 'shiny gold'), bags))))

def count_contained(container):
    return sum((i[1] * (1 + count_contained(i[0]))) for i in bags[container])
