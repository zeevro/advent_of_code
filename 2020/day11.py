...
seats = list(map(list, seats.splitlines()))

width = len(seats[0])
height = len(seats)

def count_occupied_1(state, cx, cy):
    ret = 0
    for x in range(max(cx - 1, 0), min(cx + 2, width)):
        for y in range(max(cy - 1, 0), min(cy + 2, height)):
            if (x, y) == (cx, cy):
                continue
            ret += state[y][x] == '#'
    return ret

def count_occupied_2(state, cx, cy):
    ret = 0
...
def simulate(occupied_counter, occupied2empty, empty2occupied):
    old_state = seats
    while 1:
        ...
        for x in range(width):
            for y in range(height):
                ...
                if c == '.':
                    continue
                adj = occupied_counter(old_state, x, y)
                if c == '#':
                    if adj >= occupied2empty:
                        new_state[y][x] = 'L'
                        changed = True
                elif adj <= empty2occupied:
                    new_state[y][x] = '#'
                    changed = True

        old_state = new_state

        # print('\n'.join(map(''.join, new_state)))
        # print()

        if not changed:
            break

    return sum(row.count('#') for row in old_state)

print(simulate(count_occupied_1, 4, 0))
print(simulate(count_occupied_2, 5, 0))
